...
def experiment_main():
	...
	* We print out the rate at which features occur in the top three features
	...
	df.to_csv('./RESULTS/cc/lime_exps_ml1.csv')

	stored_res1 = json.dumps(store_results1)
	with open('./RESULTS/cc/CC_LIME_one.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/rot_exps_ml1.csv')
	with open('./RESULTS/cc/CC_LIME_rot_one.json', 'w') as file:
	...
	for i in range(xtest.shape[0]):
		explanations.append(adv_explainer.explain_instance(xtest[i], adv_lime.predict_proba).as_list())
		df_rows.append({_ft.split('=')[0]: _imp for _ft, _imp in explanations[-1]})
		store_results2.append(experiment_summary([explanations[-1]], features))
	lime2_t3 = time.time()
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/lime_exps_ml2.csv')

	stored_res2 = json.dumps(store_results2)
	with open('./RESULTS/cc/CC_LIME_two.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/rot_exps_ml2.csv')
	with open('./RESULTS/cc/CC_LIME_rot_two.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/shap_exps_ms1.csv')
	
	stored_res3 = json.dumps(store_result3)
	with open('./RESULTS/cc/CC_SHAP_one.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/rot_exps_ms1.csv')
	stored_rot3 = json.dumps(rot_results3)
	with open('./RESULTS/cc/CC_SHAP_rot_one.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/shap_exps_ms2.csv')
	stored_res4 = json.dumps(store_results4)
	with open('./RESULTS/cc/CC_SHAP_two.json', 'w') as file:
	...
	df = pd.DataFrame(df_rows)
	df.to_csv('./RESULTS/cc/rot_exps_ms2.csv')
	stored_rot4 = json.dumps(rot_results4)
	with open('./RESULTS/cc/CC_SHAP_rot_two.json', 'w') as file:
		file.write(stored_rot4)

	# timestamps = {'shap2_t1': shap2_t1, 'shap2_t2': shap2_t2, 'shap2_t3': shap2_t3, 'rot2_t1': rot2_t1, 'rot2_t2': rot2_t2, 'rot2_t3': rot2_t3}
	with open('./RESULTS/cc/cc_timestamps.txt', 'w') as file:
		...
		# for var_name, ts in timestamps.items():
		# 	file.write(var_name + ': ' + str(ts) + '\n')
