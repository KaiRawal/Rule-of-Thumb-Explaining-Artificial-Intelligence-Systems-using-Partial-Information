...
class RoT(torch.nn.Module):
    ...
    def training_loop(self,loss,points,classifier_response,optimiser,epochs=1,batch_size=200,scheduler=False):
        ...
        for e in range(epochs):
            ...
            for i in range(points.shape[0] // batch_size + 1):
                ...
                optimiser.zero_grad()
                # self.project()
            if e < 32:# e % 20 == 1:  # print loss every 10 epochs
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 64 and e%2 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 128 and e%4 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 256 and e%8 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 512 and e%16 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 1024 and e%32 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 2048 and e%64 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 4096 and e%128 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            elif e < 8192 and e%256 == 1:
                print(f'\t\tEpoch: {e}, Loss: {self.training_loss[e]}')
            else:
                pass
            if e>burn_in:
    ...
    def fit(self,points,classifier_response,epochs,batch_size,lr=1e-4):
        ...
        optimiser = torch.optim.AdamW(self.parameters(), lr=lr, weight_decay=0.002)
...
class RoT_text(RoT):
    ...
    def importance(self,points):
        ...
        # print(f'{points[:,None].shape=}')
        # print(f'{self.a[None,:,None,:].shape=}')
        # print(f'{points[:,None]+self.b[None,:,:,None].shape=}')
        feature_dim = 2
        mask = (points.sum(dim=feature_dim) != -points.shape[feature_dim]).float()
        # print(f'{imp.shape=}')
        # print(f'{mask.shape=}')
        # print(f'{mask[:,None,:,None].shape=}')
    ...
    def stochastic_importance(self,points):
        ...
        # # version 2 - dropout token+f
        document_dim=2
        mask=(torch.rand(points.shape[0],points.shape[token_dim],points.shape[document_dim])>self.dropout_rate).float()
        return mask[:,None,:,:]*imp # put none in correct dimension to match importance shape
